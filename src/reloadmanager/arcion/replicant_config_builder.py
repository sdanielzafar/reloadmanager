...
class ReplicantConfigBuilder(ABC, SecretMixin):
    """
    ...
    The various files do different things:
     ...
     This class should automate the creation of these files
    """
    ...
    def _write_src_config(self) -> str:
        ...
        file_path: str = os.path.join(self.config_dir_path, f"{self.id}_source.yaml")
    ...
    def _write_target_config(self) -> str:
        ...
        target_yaml: str = textwrap.dedent(f"""
            ...
            retry-wait-duration-ms: 60000 #Duration replicant should wait before performing then next retry
        ...
        file_path: str = os.path.join(self.config_dir_path, f"{self.id}_target.yaml")
        with open(file_path, "w") as file:
    ...
    def _write_applier_config(self) -> str:
        ...
        file_path: str = os.path.join(self.config_dir_path, f"{self.id}_applier.yaml")
        with open(file_path, "w") as file:
    ...
    def _write_allow_config(self) -> str:
        ...
        file_path: str = os.path.join(self.config_dir_path, f"{self.id}_allow.yaml")
        with open(file_path, "w") as file:
    ...
    def _write_map_config(self) -> str:
        ...
        file_path: str = os.path.join(self.config_dir_path, f"{self.id}_map.yaml")
        with open(file_path, "w") as file:
            file.write(map_yaml)
