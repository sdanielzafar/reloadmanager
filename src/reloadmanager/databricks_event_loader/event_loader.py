...
from reloadmanager.databricks_event_loader.event_loader_thread import EventLoaderThread
from reloadmanager.databricks_event_loader.event_queue import EventQueue
...
class EventLoader(LoggingMixin):
    def __init__(self,
                 ...
                 metadata_table: str = "reloadmanager.table_metadata",
                 queue_schema: str = "reloadmanager",
                 lock_rows: bool = True):
        ...
        self.queue: EventQueue = EventQueue(queue_schema)
        ...
        self.dbx_client: GenericDatabaseClient = get_dbx_client()
    ...
    def get_table_metadata(self, tables: set[str]):
        table_info: list[tuple] = self.dbx_client.query(
    ...
    def update_priority(self, queued_tables: list[QueueRecord], new_tables: list[TrackerRecord]) -> list[QueueRecord]:
        ...
        tbl_metadata: dict[str, TableAttrRecord] = self.get_table_metadata(tables)
    ...
    def create_workers(self, strategy, count) -> list[EventLoaderThread]:
        if count > 0:
            ...
            threads = [
                EventLoaderThread(i, strategy, self.run_name, self.queue_schema, self.stop_signal)
